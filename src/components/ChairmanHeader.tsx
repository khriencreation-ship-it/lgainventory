...
export default function ChairmanHeader({ user }: ChairmanHeaderProps) {
  const pathname = usePathname();

  // Determine page title based on pathname
  const getPageTitle = (path: string) => {
    if (path === '/dashboard/chairman') return 'Dashboard Home';
    if (path.startsWith('/dashboard/chairman/clients')) return 'Registered Clients';
    if (path.startsWith('/dashboard/chairman/demand-bills')) return 'Demand Bills';
    if (path.startsWith('/dashboard/chairman/receipts')) return 'Receipts Ledger';
    if (path.startsWith('/dashboard/chairman/reports')) return 'Analytics Reports';
    if (path.startsWith('/dashboard/chairman/attendance')) return 'Staff Attendance Log';
    if (path.startsWith('/dashboard/chairman/profile')) return 'Profile & Signature';
    return 'LGA Chairman Portal';
  };
      ...
      <div className="flex items-center gap-5">
        ...
        >
          ...
          <div className="text-left">
            ...
            <span className="block text-[9px] font-extrabold text-indigo-700 uppercase tracking-wider mt-0.5">LG Chairman</span>
          </div>
        </Link>
      </div>
    </header>
