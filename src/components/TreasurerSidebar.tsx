...
export default function TreasurerSidebar({ user, lg }: TreasurerSidebarProps) {
  ...
  const navItems = [
    { name: 'Dashboard', href: '/dashboard/treasurer', icon: LayoutDashboard },
    { name: 'Clients', href: '/dashboard/treasurer/clients', icon: Users },
    { name: 'Demand Bills', href: '/dashboard/treasurer/demand-bills', icon: FileText },
    { name: 'Receipts', href: '/dashboard/treasurer/receipts', icon: Receipt },
    { name: 'Reports', href: '/dashboard/treasurer/reports', icon: BarChart3 },
  ];

  const isRouteActive = (href: string) => {
    if (href === '/dashboard/treasurer') {
      return pathname === '/dashboard/treasurer';
    }
      ...
      <header className="md:hidden bg-white border-b border-slate-150 px-6 py-4 flex items-center justify-between z-30">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <span className="text-[9px] text-indigo-650 font-bold uppercase tracking-wider block">Treasurer Workspace</span>
          </div>
        </div>
        <button 
              ...
              <div className="grid grid-cols-1 gap-1.5">
                {status === 'clocked_out' ? (
                  ...
                  <>
                    ...
                    >
                      {actionLoading ? <Loader2 className="h-3.5 w-3.5 animate-spin text-rose-600" /> : <LogOut className="h-3.5 w-3.5" />}
                      Clock Out
                    </button>
                  </>
                )}
